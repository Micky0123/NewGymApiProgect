...
using static BLL.CreateTrainingPlan2;
    ...
    public class CreateTrainingPlan2
    {
            ...
            IPlanDayDAL planDayDAL)
        {
            ...
            var configTaskConverter = new MapperConfiguration(cfg =>
            {
                ...
                cfg.CreateMap<TrainingPlan, TrainingPlanDTO>().ReverseMap(); // הוסף את השורה הזו!
        ...
        ///// <summary>
        ///// שליפת תרגילים לשריר וסוג מסוימים עם הגבלת ציוד
        ///// </summary>
        ///// <param name="MuscleName">שם השריר</param>
        ///// <param name="TypeMuscle">סוג השriר</param>
        ///// <param name="count">מספר התרגילים</param>
        ///// <param name="allowedEquipment">רשימת ציוד מותר</param>
        ///// <returns>רשימת תרגילים מתאימים</returns>
        //public async Task<List<ExerciseDTO>> GetExercisesForMuscleAndTypeAsync(
        //    string MuscleName,
        //    string TypeMuscle,
        //    int count,
        //    List<int> allowedEquipment)
        //{
        //    try
        //    {
        //        logger.LogInformation($"Fetching exercises for muscle: {MuscleName}, type: {TypeMuscle}");

        //        var exercises = await muscleDAL.GetExercisesForMuscleAndTypeAsync(
        //            MuscleName,
        //            TypeMuscle,
        //            allowedEquipment);
        ...
        //        logger.LogInformation($"Found {exerciseDTOs.Count} exercises for muscle: {MuscleName}, type: {TypeMuscle}");
        //        return exerciseDTOs;
        //    }
        //    catch (Exception ex)
        //    {
        //        logger.LogError(ex, $"Error while fetching exercises for muscle: {MuscleName}, type: {TypeMuscle}");
        //        throw;
        //    }
        //}
        #endregion
            ...
            object time)
        {
            ...
            try
            {
                ...
                var trainingPlan = await CreateMainTrainingPlan(traineeID, programName, daysInWeek, goal, level, time);
                ...
                // שלב 4: קישור התוכנית לפרופיל המתאמן
                //await LinkProgramToTrainee(traineeID, trainingPlan.TrainingPlanId);
                //logger.LogInformation($"Training program linked to trainee {traineeID} successfully");
                // שלב 5: שמירת מטא-דאטה נוספת
                //await SaveAdditionalMetadata(trainingPlan.TrainingPlanId, trainingParams);
                //logger.LogInformation("Training program saved completely");
            }
            catch (Exception ex)
            {
            ...
            object time)
        {
            try
            {
                ...
                //Description = $"תוכנית אימון מותאמת אישית - {daysInWeek} ימים בשבוע"
                ...
                //var savedPlan = await trainingPlanDAL.AddTrainingPlanAsync(trainingPlanDAL);

                logger.LogDebug($"Main training plan created");
                //var savedPlan= await trainingPlanDAL.GetTrainingPlanByIdAsync(traineeID);
            ...
            try
            {
                for (int dayIndex = 0; dayIndex < listOfProgram.Count; dayIndex++)
                {
                    ...
                    var planDay = new PlanDayDTO
                    {
                        ...
                        //DayName = $"יום {dayIndex + 1}",
                        //EstimatedDuration = CalculateEstimatedDuration(dayExercises),
                        //MuscleGroups = GetMuscleGroupsForDay(dayExercises),

                    };
                    PlanDay planD = mapper.Map<PlanDayDTO, PlanDay>(planDay);
                    var savedDay = await planDayDAL.AddPlanDayAsync(planD);
                    var t = await planDayDAL.GetPlanDayByIdAsync(savedDay);
                    //TrainingPlan savedPlan = await trainingPlanDAL.GetTrainingPlanByIdAsync(traineeID);
                    planDays.Add(mapper.Map<PlanDayDTO>(t));
                    //planDays.Add(savedDay);
        ...
        private List<List<DayEntry>> ExtractDayListsNew(IXLWorksheet worksheet, int daysInWeek, int time)
        {
            ...
            //// TODO: יש להוסיף את הלוגיקה לשליפת הימים לפי הפרמטרים
            //// הלוגיקה תלויה במבנה הגיליון הספציפי

            //logger.LogInformation($"Extracting day lists for {daysInWeek} days, {time} minutes");

            //// זהו מקום שבו צריך להוסיף את הלוגיקה הספציפית
            //// לשליפת תכנון האימונים היומי מהגיליון
